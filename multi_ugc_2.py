...
def build_components_from(X_feat, A_dense, aproj_dim, s=1, project_adj=True):
    """Build the normalized hashing components (X, A_proj, A_raw, hop=A.X) for an
    ARBITRARY feature matrix and dense adjacency.
    ...
    `project_adj`:
      True  (level 1)  -> the huge N-dim ORIGINAL adjacency is compressed with a
                          random JL projection to `aproj_dim`, exactly as before.
      False (level>=2) -> the coarsened adjacency A_c is used DIRECTLY as the
                          structural component (no JL re-projection). A_c is
                          already small (m x m) and its entries are precisely the
                          current supernode connection strengths, which a random
                          projection would scramble. In this case A_proj == A_raw
                          (the normalized A_c rows), so modes that use A_proj feed
                          the real coarsened connectivity into the hash.
    """
    if project_adj:
        # JL-compress the original adjacency; the SINGLE normalization (the
        # F.normalize below) is then applied -- no extra standardize step, so
        # A_proj is normalized exactly once, like every other component.
    ...
    # ONE normalization for every component, every level: row-wise L2 normalize.
    return {
...
def precompute_components(data, aproj_dim, s=1):
    ...
    The original adjacency IS JL-projected here (project_adj=True).
    """
    N = data.num_nodes
    A = to_dense_adj(data.edge_index, max_num_nodes=N).squeeze(0)
    return build_components_from(data.x, A, aproj_dim, s=s, project_adj=True)
...
def run_single(data, comps, num_classes, mode, scheme_name, h_f, args, device):
    ...
    for lvl, t_rr in enumerate(per_stage_rr, start=1):
        ...
        # project_adj=False -> A_c is used directly (NOT JL-projected again), so
        # the real supernode connection strengths feed the next hash.
        if lvl < n_stages:
            comps_cur = build_components_from(
                z_c, A_c_dense, args.aproj_dim, s=1, project_adj=False
            )
