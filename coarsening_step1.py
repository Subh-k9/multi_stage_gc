...
# Homophily-coefficient schemes.  h = homophily.
# Each returns (coeff_X, coeff_A, coeff_Ax).
SCHEMES = {
    # RECOMMENDED for both regimes: the homophily adaptation now lives inside the
    # propagation FILTER (low-pass vs high-pass, see adaptive_propagation), so the
    # block weights are kept balanced and the propagation term is NOT zeroed out
    # on heterophilic graphs. X always contributes, structure (A) is kept, and
    # the adaptive propagation contributes in both regimes.
    "adaptive":        lambda h: (1.0,   1.0,   1.0),
    # original: X*(1-h), adjacency*h, Ax*h   (kept for comparison)
...
def adaptive_propagation(X_feat, A_dense, homophily, prop_mode="adaptive"):
    """Homophily-adaptive one-hop propagation -- the term that lets the pipeline
    work on BOTH homophilic and heterophilic graphs.

    Builds the symmetric-normalized adjacency  A_norm = D^-1/2 A D^-1/2  and from
    it a low-pass (smoothing) signal and a high-pass (sharpening) signal:

        low  = A_norm @ X            smoothing  -> good when HOMOPHILIC
        high = X - A_norm @ X        sharpening -> good when HETEROPHILIC
                                     ( = (I - A_norm) X, the normalized-Laplacian
                                       filter; captures how a node DIFFERS from
                                       its neighbourhood )

    prop_mode:
      "adaptive" -> h*low + (1-h)*high   (default). Low-pass when h->1, high-pass
                    when h->0, smoothly blended in between. One knob, both regimes.
      "low"      -> low                  (degree-normalized smoothing only)
      "raw"      -> A_dense @ X_feat      (the ORIGINAL unnormalized A.X behaviour)
    """
    if prop_mode == "raw":
        return A_dense @ X_feat

    deg = A_dense.sum(dim=1, keepdim=True)
    d_inv_sqrt = torch.where(deg > 0, deg.pow(-0.5), torch.zeros_like(deg))
    A_norm = d_inv_sqrt * A_dense * d_inv_sqrt.transpose(0, 1)  # D^-1/2 A D^-1/2
    low = A_norm @ X_feat

    if prop_mode == "low":
        return low

    high = X_feat - low                      # (I - A_norm) X  -> high-pass
    h = float(homophily)
    return h * low + (1.0 - h) * high        # adaptive blend


def build_components_from(X_feat, A_dense, aproj_dim, s=1, project_adj=True,
                          normalize=True, homophily=0.5, prop_mode="adaptive"):
    """Build the hashing components (X, A_proj, A_raw, hop) for an ARBITRARY
    ...
    previous level, so the propagation term is exactly the "A_c Z_c" analogue.
    ...
    `normalize`:
      True  (level 1)  -> row-wise L2 normalize every block. The raw inputs
                          ...
                          normalization.
      False (level>=2) -> NO second normalization (see earlier note).

    `homophily`, `prop_mode`:
      control the adaptive propagation term `hop` (see adaptive_propagation).
      `homophily` is the edge-homophily of the ORIGINAL graph, reused at every
      level so the same filter is applied throughout the cascade.
    """
    if project_adj:
    ...
    # homophily-adaptive propagation (low-pass / high-pass blend) -- replaces the
    # old pure-smoothing  hop = A_dense @ X_feat  so heterophilic graphs are
    # handled by a high-pass filter instead of harmful averaging.
    hop = adaptive_propagation(X_feat, A_dense, homophily, prop_mode=prop_mode)
...
def precompute_components(data, aproj_dim, s=1, homophily=0.5, prop_mode="adaptive"):
    """Compute and normalize the level-1 building blocks once per dataset.

    Returns a dict with normalized X, A_proj, A_raw, hop. All on CPU. The
    original adjacency IS JL-projected here, the raw inputs get their single
    row-wise L2 normalization, and `hop` is the homophily-adaptive propagation.
    """
    N = data.num_nodes
    ...
    return build_components_from(data.x, A, aproj_dim, s=s,
                                 ...
                                 homophily=homophily, prop_mode=prop_mode)
    ...
    for lvl, t_rr in enumerate(per_stage_rr, start=1):
        ...
        if lvl < n_stages:
            comps_cur = build_components_from(
                ...
                homophily=h_f, prop_mode=args.prop_mode,
            )
...
def main():
    ...
    parser.add_argument("--out", default="./coar_sen",
    ...
    parser.add_argument("--prop_mode", default="adaptive",
                        choices=["adaptive", "low", "raw"],
                        help="Propagation term used in the hashing features. "
                             "'adaptive' (default) blends a low-pass and a "
                             "high-pass filter by the graph's homophily, so it "
                             "works for BOTH homophilic (smoothing) and "
                             "heterophilic (sharpening) graphs. 'low' is "
                             "degree-normalized smoothing only; 'raw' is the "
                             "original unnormalized A.X behaviour.")
